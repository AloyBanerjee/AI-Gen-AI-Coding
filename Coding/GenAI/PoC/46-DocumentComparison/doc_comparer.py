...
#                        config=config)
llm = ChatGroq(
    model_name="llama3-70b-8192",#"mixtral-8x7b-32768",  # or another Groq model like "llama2-70b-4096"
    temperature=0,
    max_tokens=8191,
...
def llm_compare(prompt_data) -> str:
    """
    This function uses Grok to create a list of differences between each uploaded document.
    :param prompt_data: Final prompt containing semantically similar prompts and the two documents to compare.
    :return: A string containing a list of the differences between the two PDF documents.
    """
    # Using Groq's chat interface directly through LangChain
    response = llm.invoke(prompt_data)
    # Grok returns a message object; we extract the content
    answer = response.content
    return answer

# def llm_compare_bedrock(prompt_data) -> str:
#     """
...
def prompt_finder(question):
    ...
    local_embeddings = OllamaEmbeddings()#HuggingFaceEmbeddings(model_name="sentence-transformers/all-MiniLM-L6-v2")
...
def doc_compare(uploaded_file_1, uploaded_file_2) -> str:
    ...
    #         \n\nAssistant:"""
    prompt = f"""\n\nHuman: Please compare Document A and Document B sentence by sentence, listing every change that took place. 
        Present the results in a Markdown table with the following columns:
        - Sentence Number: The sentence number where the change occurs
        - Document A Text: The original text from Document A
        - Document B Text: The modified text from Document B
        - Change Description: A description of what changed with proper highlighting (e.g., word replaced, added, or removed)
