...
st.set_page_config(
    page_title="Your All in One Companion",
    page_icon="📰",
    layout="wide",
...
st.title("📰 Your All in One Companion Agent")
...
with st.sidebar:
    st.header("Companion Search Topic")
    user_topic = st.text_input(
        "Enter a topic to explore:",
    )

    start_button = st.button("Start Exploration", type="primary", disabled=not user_topic)

    st.divider()
    st.subheader("Example Topics")
    example_topics = [
        "What are the must-visit cultural festivals in India that travelers should experience?",
        "Which Indian cities offer the best blend of history, food, and modern experiences for tourists?",
        "How can a first-time traveler navigate India's diverse transportation system efficiently?"
    ]
...
tab1, tab2 = st.tabs(["Exploration Process", "Detailed Report"])
