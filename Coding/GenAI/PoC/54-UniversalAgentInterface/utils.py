...
async def initialize_session_state():
    ...
    if 'user_id' not in st.session_state:
        st.session_state['user_id'] = "Ava"
    if 'model_id' not in st.session_state:  
        st.session_state['model_id'] = "anthropic:claude-3-7-sonnet-latest"
    if 'tools' not in st.session_state:
        st.session_state['tools'] = []
    if 'agents' not in st.session_state:
        st.session_state['agents'] = []
    if 'uagi_config' not in st.session_state:
        st.session_state['uagi_config'] = None
...
async def selected_model() -> str:
    ...
    model_options = {        
        ...
        "llama3-70b-8192": "groq:meta-llama/llama-3-70b-8192",
        "llama-3-70b-4096": "groq:meta-llama/llama-3-70b-4096",
        "gemma2-9b-iter": "google:gemma2-9b-it",
...
async def selected_tools() -> List[str]:
    ...
    tool_options = {
        ...
        "Python Execution": "python_tools",
        "Stock Data (Yahoo Finance)": "yfinance",
        "Calculator": "calculator",
        "SQL Querying (DuckDB)": "duckdb",
        "CSV Toolkit": "csv_tools",
...
async def selected_agents() -> List[str]:
    ...
    agent_options = {
        ...
        "Medical QA Agent": "medical_qa_agent",
        "Document QA Agent": "document_qa_agent",
        "FDA Reportability Agent": "fda_reportability_agent",
        "Knowledge Graph Agent": "knowledge_graph_agent",
        "RAG Search Agent": "rag_search_agent",
        "CER Summarizer Agent": "cer_summarizer_agent",
        "Regulatory Strategist Agent": "regulatory_strategist_agent",
        "Technical Writer Agent": "technical_writer_agent",
        "Risk Manager Agent": "risk_manager_agent",
        "Scientific Reviewer Agent": "scientific_reviewer_agent",
        "QMS Auditor Agent": "qms_auditor_agent",
    }
...
async def knowledge_widget(uagi: Team) -> None:
    ...
    if uagi is not None and uagi.knowledge is not None:
        ...
        if uploaded_file is not None:
            alert = st.info("Processing document...", icon="🧠")
        ...
        if st.button("🗑️ Delete Knowledge"):
