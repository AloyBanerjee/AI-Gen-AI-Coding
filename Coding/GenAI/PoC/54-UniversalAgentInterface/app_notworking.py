...
async def configuration_tab() -> None:   

    ####################################################################
    # Initialize User and Session State
    ####################################################################
    st.session_state.user_id = st.text_input(":technologist: User Id", value="Ava")

    ####################################################################
    # Select Model
    ####################################################################
    st.session_state.model_id = await selected_model()

    ####################################################################
    # Select Tools
    ####################################################################
    st.session_state.tools = await selected_tools()

    ####################################################################
    # Select Team Members
    ####################################################################
    st.session_state.agents = await selected_agents()


    ####################################################################
    # Create UAgI
    ####################################################################
    st.session_state.uagi_config = UAgIConfig(
        user_id=st.session_state.user_id, model_id=st.session_state.model_id, tools=st.session_state.tools, agents=st.session_state.agents
    )
    ...
    recreate_uagi = (
        ...
        or st.session_state.get("uagi_config") != st.session_state.uagi_config
    )
    ...
    if recreate_uagi:
        ...
        uagi = create_uagi(st.session_state.uagi_config)
        st.session_state["uagi"] = uagi
        st.session_state["uagi_config"] = st.session_state.uagi_config
    ...
    # Knowledge widget
    ####################################################################
    await knowledge_widget(st.session_state.uagi)

    ####################################################################
    # Session selector
    ####################################################################
    await session_selector(st.session_state.uagi, st.session_state.uagi_config)

    ####################################################################
    # About section
    ####################################################################
    await utilities_widget(st.session_state.uagi)
...
async def body() -> None:    
    ...
    try:
        ...
        st.session_state["session_id"] = st.session_state.uagi.load_session()
    except Exception:
    ...
    # Show user memories
    ####################################################################
    await show_user_memories(uagi_memory, st.session_state.user_id)


    ####################################################################
    # Load agent runs (i.e. chat history) from memory if messages is not empty
    ####################################################################
    chat_history = st.session_state.uagi.get_messages_for_session()
        ...
        with st.chat_message("assistant"):
            ...
            with st.spinner(":thinking_face: Thinking..."):
                ...
                try:
                    ...
                    run_response = await st.session_state.uagi.arun(
                    ...
                    if st.session_state.uagi.run_response is not None:
                        await add_message(
                            "assistant", response, st.session_state.uagi.run_response.tools
                        )
                    else:
...
async def main():
    initialize_session_state()
    tab1, tab2 = st.tabs(["📚 Configuration & Setttings", "💬 Chatbot"])
    
    with tab1:
        await configuration_tab()
    with tab2:        
        await header()
        await body()
        await about_agno()
